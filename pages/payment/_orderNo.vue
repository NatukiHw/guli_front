	...
	<div class="Page Confirm">
		...
		<form name="flowForm" id="flowForm" method="post" action="">
			...
			<div class="Finish">
				...
				<button class="fr redb" type="button" id="submitPay" @click="toPayment">去支付</button>
				<div class="clear"></div>
			</div>
		</form>
	</div>
...
<script>
	...
	export default {
		asyncData({ params }) {
			return orderApi.getOrder(params.orderNo).then(resp => {
				if(resp.success) {
		...
		methods: {
			toPayment() {
				window.location.href = `http://127.0.0.1:9020/payment/orders/${this.order.orderNo}/alipay`
			}
		}
	};
</script>